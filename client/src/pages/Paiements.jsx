...
const STATUT_STYLE = {
  ...
  CANCELLED: { bg: 'rgba(90,90,110,0.18)',  fg: '#9090A0', label: 'Annulé' },
}
  ...
  return (
    <Layout maxWidth="max-w-3xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2" style={{ color: '#ECECF0' }}>
            <Receipt size={20} style={{ color: '#818CF8' }} />Mes paiements
          </h1>
          <p className="text-sm mt-0.5" style={{ color: '#5A5A6E' }}>Historique de tes abonnements et achats.</p>
        </div>
        <Link to="/abonnement" className="btn-secondary btn-sm gap-1.5"><Sparkles size={13} />Voir les offres</Link>
      </div>

      {loading ? (
        <Loader2 size={22} className="animate-spin mx-auto my-12" style={{ color: '#5A5A6E' }} />
      ) : items.length === 0 ? (
        <div className="card p-10 text-center">
          <Receipt size={28} className="mx-auto mb-3" style={{ color: '#2A2A35' }} />
          <p className="text-sm mb-4" style={{ color: '#5A5A6E' }}>Aucun paiement pour le moment.</p>
            ...
            return (
              <div key={p.id} className="card p-3.5">
                <div className="flex items-center justify-between gap-2">
                  <span className="font-medium text-sm" style={{ color: '#ECECF0' }}>
                ...
                <div className="flex items-center justify-between mt-1.5">
                  ...
                  <span className="text-sm font-bold" style={{ color: '#ECECF0' }}>{fmtFCFA(p.montant)}</span>
                </div>
              </div>
            )
          })}
            ...
            <thead>
              <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
                {['Date', 'Description', 'Montant', 'Statut'].map((h, i) => (
                  <th key={i} className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider" style={{ color: '#5A5A6E' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {items.map(p => {
                ...
                return (
                  <tr key={p.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                    ...
                    <td className="px-4 py-3" style={{ color: '#ECECF0' }}>
                      {p.description ?? (p.plan ? `Abonnement ${p.plan}` : 'Achat')}
                      <div className="text-2xs font-mono" style={{ color: '#5A5A6E' }}>{p.reference}</div>
                    </td>
                    <td className="px-4 py-3 font-semibold" style={{ color: '#ECECF0' }}>{fmtFCFA(p.montant)}</td>
