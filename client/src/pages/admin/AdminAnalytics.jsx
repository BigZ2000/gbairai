...
const PLAN_COLORS = { FREE: '#9090A0', PRO: '#6366F1', PREMIUM: '#6366F1', ENTREPRISE: '#0EA5E9', ECOLE: '#22C55E' }
...
export default function AdminAnalytics() {
  ...
  return (
    <AdminLayout>
      <div className="mb-6">
        <h1 className="text-2xl font-bold" style={{ color: '#ECECF0' }}>Analytiques business</h1>
        <p className="text-sm mt-0.5" style={{ color: '#5A5A6E' }}>Revenus, abonnés et conversion.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="card p-5">
          <h2 className="font-semibold mb-4" style={{ color: '#ECECF0' }}>Répartition par plan</h2>
          <div className="space-y-3">
            {Object.entries(data.abonnes).sort((a, b) => b[1] - a[1]).map(([plan, n]) => (
              <div key={plan}>
                <div className="flex items-center justify-between text-xs mb-1">
                  <span style={{ color: '#ECECF0' }}>{plan}</span>
                  <span style={{ color: '#9090A0' }}>{n} ({Math.round((n / totalAbonnes) * 100)}%)</span>
                </div>
        ...
        {/* Top packs vendus */}
        <div className="card p-5">
          <h2 className="font-semibold mb-4 flex items-center gap-2" style={{ color: '#ECECF0' }}>
            <Package size={16} style={{ color: '#818CF8' }} />Packs les plus vendus
          </h2>
          {data.topPacks.length === 0 ? (
            <p className="text-sm py-8 text-center" style={{ color: '#5A5A6E' }}>Aucune vente de pack pour l'instant.</p>
          ) : (
            <div className="space-y-2">
              {data.topPacks.map((p, i) => (
                <div key={p.packId} className="flex items-center gap-3 p-2 rounded-lg" style={{ background: 'rgba(255,255,255,0.03)' }}>
                  <span className="text-sm font-bold w-5 text-center" style={{ color: '#5A5A6E' }}>{i + 1}</span>
                  <span className="text-lg">{p.emoji}</span>
                  <span className="text-sm flex-1 truncate" style={{ color: '#ECECF0' }}>{p.nom}</span>
                  <div className="text-right">
                    <div className="text-xs font-semibold" style={{ color: '#ECECF0' }}>{p.ventes} ventes</div>
                    <div className="text-2xs" style={{ color: '#5A5A6E' }}>{fmtFCFA(p.revenus)}</div>
                  </div>
                </div>
              ))}
            </div>
...
function Kpi({ icon: Icon, label, value, sub, color }) {
  return (
    <div className="card p-4">
      ...
      <p className="text-2xl font-bold" style={{ color: '#ECECF0' }}>{value}</p>
      <p className="text-2xs" style={{ color: '#5A5A6E' }}>{label}{sub ? ` · ${sub}` : ''}</p>
    </div>
  )
}
function MiniStat({ label, value }) {
  return (<div><p className="text-2xs" style={{ color: '#5A5A6E' }}>{label}</p><p className="text-sm font-bold" style={{ color: '#ECECF0' }}>{value}</p></div>)
}
