...
async function upsertPack(p, idx, { signature }) {
  ...
  const data = {
    ...
    ...params,
    // Overrides pack-level (priment sur les valeurs du mode).
    ...(p.nbManches != null ? { nbManches: p.nbManches } : {}),
    ...(p.nbQuestions != null ? { nbQuestions: p.nbQuestions } : {}),
    ...(p.tempsParQuestion != null ? { tempsParQuestion: p.tempsParQuestion } : {}),
    ...(p.pointsParQuestion != null ? { pointsParQuestion: p.pointsParQuestion } : {}),
    // Monétisation + mécaniques avancées (manches façon jeux TV).
    tier: p.tier ?? 'GRATUIT',
    prix: p.prix ?? 0,
    modeDistanciel:        p.modeDistanciel ?? false,
    malusEnabled:          p.malusEnabled ?? false,
    malusPenalite:         p.malusPenalite ?? 50,
    multiplicateurFinale:  p.multiplicateurFinale ?? 1.0,
    eliminationActive:     p.eliminationActive ?? false,
    masquerReponsesDefaut: p.masquerReponsesDefaut ?? false,
    signature: !!signature,
